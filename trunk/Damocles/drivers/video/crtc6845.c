...
void _vsetpen(coord_t coord){
	if (coord.y < 0 || coord.y > VIDEO_ROWS-1 || coord.x < 0 || coord.x > VIDEO_COLS - 1){ /* Light pen fuera de los límites */
		_vdisblepen();
	}
	else
	{
		if(!_vispendisabled())
			video[lightpen.y*VIDEO_COLS*2 + lightpen.x*2 + 1] = inv(video[lightpen.y*VIDEO_COLS*2 + lightpen.x*2 + 1]);
		video[coord.y*VIDEO_COLS*2 + coord.x*2 + 1] = inv(video[coord.y*VIDEO_COLS*2 + coord.x*2 + 1]);
		lightpen.x = coord.x;
		lightpen.y= coord.y;
	}
}
...
void _vtsetcursor(int page, coord_t coord){
	...
	if(0 > coord.x || VIDEO_COLS-1 < coord.x || coord.y < 0 || VIDEO_ROWS-1 < coord.y){ /* Estoy fuera de la pantalla */
		return;
	}
...
int _vcopy(void *buff, coord_t begin, coord_t end, int max){
	...
	(begin.x <= end.x)? (xstart = begin.x, xend = end.x) : (xstart = end.x, xend = begin.x);
	(begin.y <= end.y)? (ystart = begin.y, yend = end.y) : (ystart = end.y, yend = begin.y);

	for(i = 0; ystart <= yend && i < max; ystart++ )
	{
		for(xorg = xstart ; xorg <= xend && i < max ; xorg++, i++)
			bufferOut[i] = videopages[activePage][ystart*VIDEO_COLS*2 + xorg*2];
		bufferOut[i++] = '\n';
	}
...
static void pageScroll(int page){
		...
		for (i = 1 ; i < VIDEO_ROWS ; i++)
			for (j = 0 ; j < VIDEO_COLS ; j++)
				videopages[page][(i-1)*VIDEO_COLS*2 + 2*j] = videopages[page][i*VIDEO_COLS*2 + 2*j];
...
void _vtcflush(int page, void *buff, size_t qty){
	...
	for(i = 0 ; i < qty ; i++){
		...
		default:
			...
			videopages[page][(coord.y*VIDEO_COLS+ coord.x)*2] = bufferIn[i];
			coord.x++;
			break;
		}
...
void _vtflush(int page, void *buff, size_t qty){
	...
	for (i = 0 ; i < qty ; i++)
	{
		switch(buffer[i]){
			...
			default:
				renderline[24] = 1;
				videopages[page][24*VIDEO_COLS*2 + 2*(linepos[page]++)] = buffer[i];
				break;
...
static void rerenderVideo(int page){
	...
	if(page == activePage){
		...
		if(!_vispendisabled())
			video[lightpen.y*VIDEO_COLS*2 + lightpen.x*2 + 1] = inv(video[lightpen.y*VIDEO_COLS*2 + lightpen.x*2 + 1]); //Coloco el cursor donde estaba
	}

	for(i = 0 ; i < VIDEO_ROWS ; i++) renderline[i] = 0 ;
	scrolled = 0;
}
